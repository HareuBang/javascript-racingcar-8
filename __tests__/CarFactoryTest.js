import CarFactory from "../src/model/CarFactory.js";
...
describe("CarFactory를 통해 생성된 Car 동작 확인", () => {
  describe("자동차 생성 기능 테스트", () => {
    test.each([
      ...
      (inputName) => {
        const car = CarFactory.create(inputName);
  ...
  describe("자동차 전진 기능 테스트", () => {
    test("생성된 자동차를 전진시키면 이동거리가 1 증가합니다.", () => {
      const car = CarFactory.create("pobi");
  ...
  describe("예외 테스트", () => {
    ...
    test.each([
      ...
      (name) =>
        expect(() => CarFactory.create(name)).toThrow(CAR_NAME_ERROR_NOT_STRING)
    );

    test.each([[""], ["     "]])(
      `잘못된 자동차 이름 "%s"을(를) 입력하면 ${CAR_NAME_ERROR_NOT_EMPTY}에러를 반환한다.`,
      (name) => {
        expect(() => CarFactory.create(name)).toThrow(CAR_NAME_ERROR_NOT_EMPTY);
      }
    );
    ...
    test.each([["123456"], ["이것은 자동차 이름입니다."]])(
      ...
      (name) => {
        expect(() => CarFactory.create(name)).toThrow(
